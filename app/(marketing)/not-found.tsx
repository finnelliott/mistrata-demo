...
export default function NotFound() {
    // const treatments = use(fetch(`${process.env.NEXT_PUBLIC_VERCEL_URL}/api/pages?where[slug][equals]=${"treatments"}`).then((res) => res.json()).then((res) => res.docs[0]))
    // const blog = use(fetch(`${process.env.NEXT_PUBLIC_VERCEL_URL}/api/pages?where[slug][equals]=${"blog"}`).then((res) => res.json()).then((res) => res.docs[0]))
    // const about = use(fetch(`${process.env.NEXT_PUBLIC_VERCEL_URL}/api/pages?where[slug][equals]=${"about"}`).then((res) => res.json()).then((res) => res.docs[0]))
    // const contact = use(fetch(`${process.env.NEXT_PUBLIC_VERCEL_URL}/api/pages?where[slug][equals]=${"contact"}`).then((res) => res.json()).then((res) => res.docs[0]))
